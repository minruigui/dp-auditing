...
def bkd_find_thresh(nobkd_li, bkd_li, use_dkw=False):
    ...
    for thresh in all_threshs:
        ...
        print("plain_ratio", plain_ratio, np.log(plain_ratio))
        plain_eps = np.log(plain_ratio)
        print("corr_ratio", corr_ratio, np.log(corr_ratio))
        ...
        # print("best", best_corr_thresh)
    return best_corr_thresh[1]


def bkd_find_thresh_(nobkd_li, bkd_li, use_dkw=False):
    # find the biggest ratio
    best_threshs = {}
    nobkd_arr = nobkd_li
    bkd_arr = bkd_li
    ...
    plain_eps = np.log(plain_ratio)
    # print("log", corr_ratio, np.log(corr_ratio))
    corr_eps = np.log(corr_ratio)
...
for cfg in valid_cfgs:
    ...
    best_thresh_ = bkd_find_thresh_(nobkd_search, bkd_search, use_dkw=True)
    print(best_thresh,best_thresh_)
...
print("results:", os.path.join(res_dir, "results"),vals)
